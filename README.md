...
- **Search** — Find in Note highlights every match, with a lighter highlight for the current one. Vault search lists individual occurrences ranked across notes, with their line numbers and context. Results start unselected; ↑/↓ previews the exact match in the main pane, Enter keeps it, and Esc restores the starting note, selection, and scroll position. Both searches show the current match and total.
...
- **Pointer-friendly dialogs** — Settings, confirmations, galleries, file/color pickers, update messages, and other dialogs leave the desktop pointer free instead of using a compositor-level modal grab.
- **Stable and Development updates** — choose a release channel under **Settings → Updates**. Emerald quietly checks that channel once at startup and notifies you only when a newer release is available; **Check for Updates…** remains available for an immediate manual check. Stable remains the default; Development receives early test builds and later stable releases. Verified updates install and restart automatically on macOS and Linux, with download percentage shown clearly beside the progress bar; native Linux builds are promoted to a self-contained per-user AppImage at `~/.local/bin/emerald`, with no `sudo`. After an upgrade, Emerald opens an offline **What's New** panel once; the same notes remain available from the gear menu.
